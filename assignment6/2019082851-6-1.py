...
def render():
    ...
    xang = np.radians(gGamma)
    yang = np.radians(gBeta)
    zang = np.radians(gAlpha)
    M = np.identity(4)
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight,gAlpha,gBeta,gGamma,gAlpha_init,gBeta_init,gGamma_init
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'20190852851-6-1', None,None)
    if not window:
        glfw.terminate()
        return
